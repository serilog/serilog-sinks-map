...
public static class MapLoggerConfigurationExtensions
{
        ...
        LoggingLevelSwitch? levelSwitch = null)
    {
        ...
        {
            if (le.Properties.TryGetValue(keyPropertyName, out var v) &&
                v is ScalarValue { Value: TKey key })
            {
                return key;
            }
        ...
        LoggingLevelSwitch? levelSwitch = null)
    {
        ...
        if (configure == null) throw new ArgumentNullException(nameof(configure));
        if (sinkMapCountLimit is < 0) throw new ArgumentOutOfRangeException(nameof(sinkMapCountLimit));

        return loggerSinkConfiguration.Sink(
            new MappedSink<TKey>(
                // ReSharper disable once UseNullableAnnotationInsteadOfAttribute
                (LogEvent logEvent, [MaybeNull] out TKey key) =>
                {
